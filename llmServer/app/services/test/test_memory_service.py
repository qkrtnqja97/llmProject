# app/services/test/test_memory_service.py
# PYTHONPATH=. python -m app.services.test.test_memory_service
...
async def run_memory_full_test():
    ...
    rebuilt_q, structured_memory = await memory.inject_context(
        user_id=user_id,
    ...
    rebuilt_q2, structured_memory2 = await memory.inject_context(
        user_id=user_id,
        question=pronoun_test,
    )

    final_q2 = inject_memory_to_question(rebuilt_q2, structured_memory2)

    print("입력 질문:", pronoun_test)
    print("재조립 결과:", rebuilt_q2)
